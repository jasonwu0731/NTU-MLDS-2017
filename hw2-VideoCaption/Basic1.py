...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        print 'self.lstm1.state_size',self.lstm1.state_size #LSTMStateTuple(c=256, h=256)
        ...
        for i in range(self.n_lstm_steps): ## Phase 1 => only read frames
            ...
            with tf.variable_scope("LSTM2"):
                output2, state2 = self.lstm2( tf.concat(values=[padding, output1], axis=1), state2 )
        ...
        for i in range(self.n_lstm_steps): ## Phase 2 => only generate captions
            ...
            with tf.variable_scope("LSTM2"):
                output2, state2 = self.lstm2( tf.concat(values=[current_embed, output1], axis=1), state2 )

            labels = tf.expand_dims(caption[:,i], 1)
            indices = tf.expand_dims(tf.range(0, self.batch_size, 1), 1)
            concated = tf.concat(values=[indices, labels], axis=1)
            onehot_labels = tf.sparse_to_dense(concated, tf.stack([self.batch_size, self.n_words]), 1.0, 0.0) #tf.pack has been renamed as tf.stack.
...
video_feat_path = 'MLDS_hw2_data/training_data/feat/'
train_label_file = 'MLDS_hw2_data/training_label.json'
...
def get_video_data(video_feat_path, train_label_file, train_ratio=0.9):
    
    video_data = pd.read_json(train_label_file)
    video_data['video_path'] = video_data['id'].map(lambda x: x+'.npy')
...
def train():
    train_data, _ = get_video_data(video_feat_path, train_label_file, train_ratio=0.9)
    captions = train_data['caption']
    captions = captions.map(lambda x: ''.join(x).replace('.', ''))
    captions = captions.map(lambda x: ''.join(x).replace(',', ''))
    ...
    for epoch in range(n_epochs):
        ...
        for start,end in zip(
            ...
            current_caption_ind = map(lambda cap: [wordtoix[word] for word in "".join(cap).lower().split(' ')[:-1] if word in wordtoix], current_captions)
